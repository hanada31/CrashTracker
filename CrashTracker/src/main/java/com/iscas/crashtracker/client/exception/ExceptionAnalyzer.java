...
public class ExceptionAnalyzer extends Analyzer {
    ...
    private void getMethodInfo() {
        ...
        String folder = MyConfig.getInstance().getResultFolder() + File.separator + Global.v().getAppModel().getAppName() + File.separator + "CodeInfo" + File.separator;
    ...
    private void getExceptionList() {
        ...
        HashSet<SootClass> applicationClasses = new HashSet<>(Scene.v().getApplicationClasses());
        for (SootClass sootClass : applicationClasses) {
//            if(!sootClass.getName().contains("Context")) continue;
                ...
                try {
                    ...
                    for(Map.Entry<SootMethod, Map<Unit,Local>> entryOuter: method2unit2Value.entrySet()) {
                        ...
                        for (Map.Entry<Unit, Local> entryInner : unit2Value.entrySet()) {
                            getThrowUnitWithType(unit2Type, sm, entryInner.getKey(), entryInner.getValue(), new ArrayList<>());
                        }
    ...
    /**
     * Generate permissionSet and write it to file
     */
    private void getPermissionSet() {
        ...
        for (SootClass sootClass : applicationClasses) {
            ...
            for (SootMethod sootMethod : sootClass.getMethods()) {
                ...
                for(Unit u: sootMethod.getActiveBody().getUnits()){
                    if (!u.toString().contains("android.permission.")) continue;
                    // str is string behind "android.permission"
        ...
        String folder = MyConfig.getInstance().getResultFolder() +File.separator+ Global.v().getAppModel().getAppName()+File.separator+"Permission"+File.separator;
        FileUtils.createFolder(folder);
     ...
     *
     * @return A map that store throw method and all throw units in this method.
     * Every throw unit will map a throw local
     */
    public Map<SootMethod, Map<Unit, Local>> getThrowUnitWithValue(SootMethod sootMethod){
        Map<SootMethod, Map<Unit, Local>> method2unit2Value = new HashMap<>();
        if(!sootMethod.hasActiveBody()) return method2unit2Value;
        ...
        for (Unit unit : sootMethod.getActiveBody().getUnits()) {
            if (unit instanceof ThrowStmt) {
                ...
                getOtherNonThrowUnits(method2unit2Value, sootMethod, unit);
            }
        }
    ...
    /**
     * except throwUnit, find out more methods that operate an exception like log method etc.
     */
    private void getOtherNonThrowUnits(Map<SootMethod, Map<Unit, Local>> method2unit2Value, SootMethod sootMethod, Unit unit) {
        ...
        for (Value arg : invoke.getArgs()) {
            ...
            List<Unit> defsOfOps = SootUtils.getDefOfLocal(sootMethod.getSignature(), arg, unit);
            if (defsOfOps.size() == 0) continue;

            Unit defOfLocal = defsOfOps.get(0);
            if (((AbstractDefinitionStmt) defOfLocal).getRightOp() instanceof JNewExpr) {
                addThrowPoint(method2unit2Value, sootMethod, unit, arg);
     ...
     */
    public void getThrowUnitWithType(Map<Unit, String> unit2Message, SootMethod sootMethod, Unit unit, Local localTemp, ArrayList history){
        List<Unit> defsOfOps = SootUtils.getDefOfLocal(sootMethod.getSignature(),localTemp, unit);
        if (defsOfOps.size() == 0) return;
        Unit defOfLocal = defsOfOps.get(0);
        if(!history.contains(defOfLocal)) {
            history.add(defOfLocal);
        }else{
            return;
        }
        if (defOfLocal instanceof DefinitionStmt) {
            ...
            if (rightValue instanceof NewExpr) {
                ...
                getThrowUnitWithType(unit2Message, sootMethod, unit, (Local) rightValue, history);
                ...
                if (s.endsWith("Exception") || s.equals("java.lang.Throwable")) {
                    ...
                    if (value instanceof Local) {
                        getThrowUnitWithType(unit2Message, sootMethod, unit, (Local) value, history);
                    }
                }
...
//                log.warn("Unimplemented condition: rightValue is CaughtExceptionRef");
                ...
                for (ValueUnitPair arg : phiExpr.getArgs()) {
                    if (arg.getValue() instanceof Local) {
                        getThrowUnitWithType(unit2Message, sootMethod, unit, (Local) arg.getValue(), history);
                    }
                }
...
//        if(excpetionInfoSootMethod.getSignature().contains("readException(int,java.lang.String)>")){
//            System.out.println();
//        }
        ...
        for (Unit u: sootMethod.getActiveBody().getUnits()) {
            if (u instanceof ReturnStmt) {
                if(!conditionOfRetIsCaughtException(sootMethod, u, new HashSet<>()))
                    retUnits.add(u);
            }
        }
        for(Unit condUnit: exceptionInfo.getConditionUnits()) {
            getRetUnitsFlowIntoConditionUnits(sootMethod, condUnit, retUnits, new HashSet<>());
        }
        ...
        exceptionInfo.setId(Global.v().getExceptionCount());
        Global.v().addExceptionCount();
        ...
        for (Iterator<Edge> it = Global.v().getAppModel().getCg().edgesInto(sootMethod); it.hasNext(); ) {
            ...
            for (SootClass sootClass : targetClasses) {
                ...
                if (StringUtils.getPkgPrefix(pkg1, 2).equals(StringUtils.getPkgPrefix(pkg2, 2))
                    ...
                    if(sm!=null) {
                        for (SootClass interfaceSC : sm.getDeclaringClass().getInterfaces()) {
            ...
            if(u instanceof  JAssignStmt){
                ...
                if(jAssignStmt.getLeftOp() instanceof  FieldRef){
                    if (field ==  jAssignStmt.getFieldRef().getField()) {
                        ...
                        assigns.addAll(nestableObj.getValues());
                    }
                }
            }
        }
        ...
        for (Iterator<Edge> it = Global.v().getAppModel().getCg().edgesInto(sootMethod); it.hasNext(); ) {
            ...
            if(edgeTgtMtd == sootMethod){
                for(Unit u: SootUtils.getUnitListFromMethod(edgeSrcMtd)){
                    ...
                    if(invoke!=null && SootUtils.isSameOrSubClassCall(invoke.getMethod(), edgeTgtMtd.method())){
                ...
                if (invokeExpr.getMethod().getDeclaringClass().toString().equals(exceptionClassName)) {
                    ...
                    if (invokeExpr.getArgCount() > 0 && StringUtils.isStringType(invokeExpr.getArgs().get(0).getType())) {
                        ...
                        if (arg instanceof Local) {
                            ...
                            exceptionInfo.setExceptionMsg(exceptionMsg.equals("")?"[\\s\\S]*":exceptionMsg);
        ...
        if (defOfLocal instanceof DefinitionStmt) {
            ...
            if (rightOp instanceof Constant) {
                ...
                if (invokeSig.equals("<java.lang.StringBuilder: java.lang.String toString()>")) {
                    ...
                    if (value instanceof Local) {
                        getMsgContentByTracingValue(exceptionInfo, sootMethod, (Local) value, defOfLocal, message);
                    }
...
//                        extendRelatedValues(sootMethod, allPreds, exceptionInfo, defOfLocal, argConstant, new ArrayList<>(), new HashSet<>(), true);
                ...
                getMsgContentByTracingValue(exceptionInfo, sootMethod, (Local) rightOp, defOfLocal, message ) ;
            }
        }
    }

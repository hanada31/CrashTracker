...
def analyzeApk(apkPath, resPath, sdk, frameworkVersion, strategy):
    ...
    if(os.path.exists(apkPath)): 
        ...
        ApkCrashAnalysisClient = extraArgs +" -strategy " +strategy +" "
        ...
        for apk in apks:
            ...
            if apk[-4:] ==".apk":
                ...
                if(reRun or not os.path.exists(resFile)): 
                    command = "java -jar "+jarFile+"  -path "+ apkPath +" -name "+apk+" -androidJar "+ sdk +"/platforms  "+ extraArgs +" -crashInput ../Files/crashInfo.json  -exceptionInput ../Files/ ApkCrashAnalysisClient -client ApkCrashAnalysisClient" +" -outputDir "+outputDir + " >> "+logDir+"/"+apk[:-4]+".txt"
                    print (command)
